...
- Legale Moments (v175 / EN v252 / consenso v267): `moments-privacy.html` / `moments-terms.html` (IT, URL invariati) + `moments-privacy-en.html` / `moments-terms-en.html`. Signup: Termini+Privacy obbligatori; **novità/offerte facoltative** (non pre-spuntate), registro SQL v175, revoca in Account → Profilo. Worker **non** toccato. **P.IVA / ragione sociale / sede: ancora da inserire** quando ci saranno dati reali — non inventare. Su `/m/` footer + notice tecnica (no cookie marketing; `visitorId` solo server-side per PIN/rate-limit).
