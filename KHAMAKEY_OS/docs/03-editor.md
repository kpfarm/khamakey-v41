...
- **v158 (incidente prod)**: non usare mai optional chaining a sinistra di un’assegnazione (`el?.disabled = …` è `SyntaxError` e ferma tutto lo script). Verificare con `node --check` lo script inline di `editor.html` prima del deploy Pages.
