...
- Legale Moments (v175 / EN v252): `moments-privacy.html` / `moments-terms.html` (IT, URL invariati) + `moments-privacy-en.html` / `moments-terms-en.html`. Signup/account/footer `/m/` puntano alla lingua UI o del visitatore. Testo IT vince se le versioni divergono. **P.IVA / ragione sociale / sede: ancora da inserire** quando ci saranno dati reali — non inventare. Checkbox Termini+Privacy in signup (gate UI, non registro). **Niente opt-in email marketing** — inventario [[34-gdpr-moments]]. Su `/m/` footer + notice tecnica (no cookie marketing; `visitorId` solo server-side per PIN/rate-limit).
