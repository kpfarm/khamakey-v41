...
### Sezioni principali
- **Ingresso editor (v274):** all’apertura si atterra su **Pagina → Riepilogo**. Percorso unico = barra **4 passi** (Copertina, Colori, Contenuti, Pubblica). Sotto: **Cosa puoi fare**, ordine **Pagina → Contenuti → Insieme → Account**. **Insieme** è solo l’invito a modificare (una persona, email). Link/QR e PIN stanno in Pagina, non in Insieme. Non chiamare le sezioni «servizi». **Non** aggiungere un secondo first-win in Riepilogo: la card «Prima cosa / Quando sei pronto» (v270) è tolta in v273 — i due testi non erano mobile vs desktop, ma `cover_url` presente o no. **v271:** sezioni ancora vuote mostrano tre passi + Non succede; spariscono quando c’è contenuto. **v272–v273 Officina:** cassetto più largo; anteprima laterale visibile su desktop (si restringe, non si nasconde sotto 1100px); mobile resta FAB. Barra bassa: Pagina · Design · Contenuti. Il QR/NFC pubblico `/m/` `/k/` resta la pagina ospite — non l’editor.
...
- Legale Moments (v176 / EN v252 / consenso v267 / diritti v268): `moments-privacy.html` / `moments-terms.html` (IT, URL invariati) + `moments-privacy-en.html` / `moments-terms-en.html`. Signup: Termini+Privacy obbligatori; **novità/offerte facoltative** (non pre-spuntate), registro SQL v175, revoca in Account → Profilo. Stesso Profilo: download JSON e cancellazione account (SQL v176). Worker **non** toccato. **P.IVA / ragione sociale / sede: ancora da inserire** quando ci saranno dati reali — non inventare. Su `/m/` footer + notice tecnica (no cookie marketing; `visitorId` solo server-side per PIN/rate-limit).
...
Limiti server: immagine 8 MB · video **Moments 90 MB** (SQL v173 / piano) · Business video **50 MB** (`MEDIA_LIMITS`) · audio 12 MB · **PDF 15 MB**.  
Oltre 90 MB Moments serve Cloudflare Business (200 MB) o upload diretto R2 — non alzare il Worker sopra 90 sul piano Free/Pro (request body 100 MB).  
