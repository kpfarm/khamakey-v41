...
const LIST_LABELS = {
  promises:{ singular:"Promessa", plural:"promesse", add:"+ Aggiungi promessa", empty:"Aggiungi le vostre promesse — emoji e testo, una card ciascuna." },
  dreams:{ singular:"Sogno", plural:"sogni", add:"+ Aggiungi sogno", empty:"Scrivi i sogni da realizzare insieme. Spunta «Realizzato» se già avete fatto il passo." },
  rituals:{ singular:"Rituale", plural:"rituali", add:"+ Aggiungi rituale", empty:"Le piccole abitudini quotidiane — una card per ogni rituale." },
  numbers:{ singular:"Numero", plural:"numeri", add:"+ Aggiungi numero", empty:"Statistiche simboliche — es. «365» e «giorni insieme»." }
};
  ...
  if(key === "promises"){
    ...
    return `<article class="list-item-card journey-step-card" data-list-key="${esc(key)}" data-list-id="${esc(safe.id)}" data-list-index="${index}">
      <header class="journey-step-head">
        <span class="journey-step-badge">${labels.singular} ${num}</span>
        <button type="button" class="journey-step-remove" data-list-remove="${esc(safe.id)}" aria-label="Elimina ${labels.singular} ${num}">Elimina</button>
      </header>
      <div class="list-item-fields journey-step-fields">
        <label>Emoji<input class="list-item-field" data-list-field="emoji" type="text" value="${esc(safe.emoji)}" maxlength="4" placeholder="💍"></label>
        <label>Testo<textarea class="list-item-field" data-list-field="text" rows="2" placeholder="Es. Esserci sempre">${esc(safe.text)}</textarea></label>
      </div>
    </article>`;
  }
  if(key === "dreams"){
    ...
    return `<article class="list-item-card journey-step-card" data-list-key="${esc(key)}" data-list-id="${esc(safe.id)}" data-list-index="${index}">
      ...
      <div class="list-item-fields journey-step-fields">
        <label class="list-item-check"><input class="list-item-field" data-list-field="done" type="checkbox" ${safe.done ? "checked" : ""}> Realizzato</label>
        <label>Testo<textarea class="list-item-field" data-list-field="text" rows="2" placeholder="Es. Viaggiare in Giappone">${esc(safe.text)}</textarea></label>
      </div>
    </article>`;
  }
  if(key === "rituals"){
    ...
    return `<article class="list-item-card journey-step-card" data-list-key="${esc(key)}" data-list-id="${esc(safe.id)}" data-list-index="${index}">
      ...
      <div class="list-item-fields journey-step-fields">
        <label>Emoji<input class="list-item-field" data-list-field="emoji" type="text" value="${esc(safe.emoji)}" maxlength="4" placeholder="☕"></label>
        <label>Testo<textarea class="list-item-field" data-list-field="text" rows="2" placeholder="Es. Caffè insieme ogni mattina">${esc(safe.text)}</textarea></label>
      </div>
    </article>`;
  }
  if(key === "numbers"){
    ...
    return `<article class="list-item-card journey-step-card" data-list-key="${esc(key)}" data-list-id="${esc(safe.id)}" data-list-index="${index}">
      ...
      <div class="list-item-fields journey-step-fields">
        <label>Numero<input class="list-item-field" data-list-field="value" type="text" value="${esc(safe.value)}" placeholder="Es. 365"></label>
        <label>Etichetta<input class="list-item-field" data-list-field="label" type="text" value="${esc(safe.label)}" placeholder="Es. giorni insieme"></label>
      </div>
    </article>`;
  }
  return "";
...
export function renderListItems(formNode,key){
  ...
  list.innerHTML = items.length
    ...
    : `<div class="journey-empty"><p><strong>Nessuna voce ancora</strong></p><p>${esc(labels.empty)}</p></div>`;
...
export function renderListItemsPanel(key,section){
  ...
  return `<div class="list-items-panel" data-list-section="${esc(key)}">
    <p class="field-hint">Tocca <strong>${labels.add.replace("+ ","")}</strong> per ogni voce — niente più «una riga per elemento».</p>
    <div class="journey-steps-head">
      <span class="journey-steps-count"><strong id="listItemCount_${esc(key)}">${items.length}</strong> / ${MAX_LIST_ITEMS} ${labels.plural}</span>
      <button type="button" class="primary journey-add-btn" data-list-add="${esc(key)}">${labels.add}</button>
    </div>
    ...
    <label class="list-intro-label">Testo introduttivo (facoltativo)<textarea name="section_${esc(key)}_body" rows="2" placeholder="Una frase sopra l'elenco…">${esc(section.body || "")}</textarea></label>
  </div>`;
}
...
export function bindListItemsEditor(formNode){
  ...
  formNode.addEventListener("click",event=>{
    ...
    if(addBtn && formNode.contains(addBtn)){
      ...
      const items = readListItems(formNode,key);
      if(items.length >= MAX_LIST_ITEMS) return alert(`Massimo ${MAX_LIST_ITEMS} voci.`);
      const mode = modeForKey(key);
