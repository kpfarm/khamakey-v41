-- KhamaKey v173 — Moments: video singolo fino a 90 MB
-- Cloudflare Free/Pro: request body max 100 MB; 90 lascia margine al multipart.
-- Non alza video_clips (Free resta 1). Business invariato.

update public.platform_plans
set
  limits = jsonb_set(coalesce(limits, '{}'::jsonb), '{max_video_mb}', '90'::jsonb, true),
  updated_at = now()
